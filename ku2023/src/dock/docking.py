...
import dock.dock_visualize_2 as dock_visualize
...
class Docking:
    def __init__(self):
        ...
        self.docking_x, self.docking_y, _ = gc.enu_convert(rospy.get_param("docking_end"))
    ...
    def print_status(self, error_angle, u_servo, u_thruster):
        ...
        if self.state in [0, 1, 2, 3]:
            ...
            error_angle_dir_str = "( Right )" if error_angle > 0 else "(  Left )"
            #u_servo = float(u_servo)
            #self.servo_middle = float(self.servo_middle)
