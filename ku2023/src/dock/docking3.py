...
class Docking:
    def __init__(self):
        ...
        self.model = YOLO('{}/find_model_20.pt'.format(path))
    ...
    def check_target(self, return_target=False):
        bboxes = []
        self.results= self.model(self.raw_img)
        ...
        cv2.imshow("controllssser", annotated_frame)
        ...
        ######edit#########
        target = []  #수정한 타겟. 분류된 클래스와 바운딩박스 넣음
        if boxes.cls.nelement() != 0:
            target_number = 4
            if int(boxes.cls.tolist()[0]) == 0: #circle
                target_number = 0
            elif int(boxes.cls.tolist()[0]) == 1: #square
                target_number = 1
            elif int(boxes.cls.tolist()[0]) == 2: #cross
                tatget_number = 2
            elif int(boxes.cls.tolist()[0]) == 3: #triangle
                target_number = 3
            print(self.target_shape,self.target_shape,self.target_shape,self.target_shape)
            print(target_number, target_number,target_number,target_number,target_number,target_number,target_number,)
            if mark_detect.is_target_fix(self.target_shape, target_number):
                target.append(boxes.cls)
                print(boxes.xywh[0].tolist()[1])
                center = []
                center.append(boxes.xywh[0].tolist()[0] + boxes.xywh[0].tolist()[2] /2)  #바운딩 박스 중간 지점 추출
                center.append(boxes.xywh[0].tolist()[1] + boxes.xywh[0].tolist()[3] /2)  
                target = [boxes.cls.tolist()[0], center]
                return_target=True
                #return_target 성공적으로 들어감
            else:
                return False
        ...
        # 표지 판단
            ...
            if docking.mark_check_cnt >= docking.target_detect_time:  
                ...
                if docking.detected_cnt >= docking.target_detect_cnt:
                    ...
                    #if docking.target[0] == (받는 입력값){} docking.target_found 이 안에 넣기 히고 else로 False 값 넣으면 됨
