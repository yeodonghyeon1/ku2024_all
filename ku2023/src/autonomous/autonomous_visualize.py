...
def visualize(ac):
    ...
    all_markers = visual.marker_array_rviz(
     ...
     # 남은 waypoints
    for idx in range(ac.waypoint_idx, len(ac.waypoints) + 1):
        # waypoints
        waypoint = visual.point_rviz(
            name="waypoints",
            id=ids.pop(),
            x=ac.waypoints[idx][0],
            y=ac.waypoints[idx][1],
            color_r=78,
            color_g=166,
            color_b=58,
            scale=0.3,
        ...
        waypoint_txt = visual.text_rviz(
            name="waypoints",
            id=ids.pop(),
            x=ac.remained_waypoints[idx][0],
            y=ac.remained_waypoints[idx][1],
            text=str(idx),
            scale=1.2,
        )
    ...
    # 통과한 waypoints
    for idx in range(1, ac.waypoint_idx):
        # waypoints
        waypoint = visual.point_rviz(
            name="waypoints",
            id=ids.pop(),
